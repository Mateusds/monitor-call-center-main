...
// Predefined safe color names (CSS Level 4 named colors)
...
// Strict validation for CSS color values to prevent CSS injection
const isValidCssColor = (color: string): boolean => {
  ...
  // Check rgb/rgba with strict numeric validation (no nested functions)
  if (/^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*(,\s*(0|1|0?\.\d+))?\s*\)$/.test(trimmed)) {
    return true;
  }
  
  // Check hsl/hsla with strict numeric validation (no nested functions)
  if (/^hsla?\(\s*\d{1,3}\s*,\s*\d{1,3}%\s*,\s*\d{1,3}%\s*(,\s*(0|1|0?\.\d+))?\s*\)$/.test(trimmed)) {
    return true;
  }
  
  // Check CSS variables with strict alphanumeric-dash pattern
  if (/^var\(--[a-z][a-z0-9-]*\)$/.test(trimmed)) {
    return true;
  }
  
  // Check safe named colors only
  if (SAFE_COLOR_NAMES.has(trimmed)) {
    return true;
  }
...
// Sanitize CSS key names to prevent injection - strict alphanumeric only
